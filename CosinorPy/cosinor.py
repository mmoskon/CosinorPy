...
def remove_baseline_comp(X, Y, width = 7, return_fit=False):
    ...
    X = np.array(X)
    Y = np.array(Y)
    kernel = gauss(width) # TODO: change this to something else (gauss, maybe?)
...
def detrend_amp(X, Y, period, phaseLoc, amps=[]):
    # as above, so below. loop through X, get indices, then coorect Y via multiplication. Feed into new Y with same dimensions as old Y
    Y_new = []
