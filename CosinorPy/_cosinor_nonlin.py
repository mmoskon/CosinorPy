...
# basic fit
def linear_model(predictor, A, B):
    X = predictor
    return A + B * X
...
def get_best_model(X, Y, period=24, n_components = [0,1,2,3], plot=False, plot_margins=True, save_to = "", **kwargs):
    
       
    best = {
        "amp_comp": False,
        "n_comps": n_components[0]
    }

    popt_best, stats_best, stats_params_best, rhythm_best = fit_generalized_cosinor_n_comp(X, Y, period = period, n_components=n_components[0], plot=False, lin_comp=True, amp_comp=False, **kwargs)
    
    
    RSS_best = stats_best['RSS']
    n_params_best = len(popt_best)
    if period:
        n_params_best -= 1
    ...
    for amp_comp in [False, True]:
        for n_comps in n_components:
            ...
            if amp_comp == best["amp_comp"] and n_comps == best["n_comps"]:
                continue

            try:
                popt2, stats2, stats_params2, rhythm_params2 = fit_generalized_cosinor_n_comp(X, Y, period = period, n_components=n_comps, plot=False, lin_comp=True, amp_comp=amp_comp, **kwargs)
            except:
                continue
            ...
            if cosinor.compare_models(RSS_best, RSS2, DoF_best, DoF2) < 0.05:                
                best["amp_comp"] = amp_comp
                best["n_comps"] = n_comps

                popt_best = popt2
                stats_best = stats2
                stats_params_best = stats_params2
                rhythm_best = rhythm_params2
                
                RSS_best = RSS2
                n_params_best = n_params2
                DoF_best = DoF2

            
        if plot:
            fit_generalized_cosinor_n_comp(X, Y, period = period, n_components=best["n_comps"], plot=True, plot_margins=plot_margins, save_to = save_to, lin_comp=True, amp_comp=best["amp_comp"], **kwargs)



    return best["n_comps"], popt_best, stats_best, stats_params_best, rhythm_best
